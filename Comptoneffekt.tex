...
Der schematische Aufbau eines Röntgendetektors ist in \autoref{fig:skizze_roetgendetektor} ersichtlich.
...
Diese Benennung ist schematisch in \autoref{fig:termschema_roentgenlinien} sichtbar.
...
Die Energie $E_n$ des Elektrons wird dabei durch folgende modifizierte Bohrsche Formel Berechnet.
...
Für die Frequenzen der charakteristischen Linien $\sigma=\left(E_2-E_1\right) / h$ bei den Übergängen $n_2->n_1\left(n_2>n_1\right)$ gilt folgender Zusammenhang:
\begin{equation}
	\nu=c R\left[\frac{\left(Z-\sigma_{n 1}\right)^2}{n_1^2}-\frac{\left(Z-\sigma_{n 2}\right)^2}{n_2^2}\right]
	\label{eq:frequenz}
\end{equation}

Wird nun eine mittlere Abschirmkonstante $\sigma_{2,1}$ für einen Übergang von $n_2$ nach $n_1$ eingeführt, ergibt sich folgendes als das Moseleysche Gesetz:
\begin{equation}
...
Setzt man $R_y=R h c \approx 2,18 \times 10^{-18} \mathrm{~J} \approx 13,6 \mathrm{eV}$ und 
betrachtet die $K \alpha$-Linien leichterer Elemente $(\mathrm{Z}<30)$ ergibt sich schließlich \cite{unterlagen_rontgenfluorenzenz}:
\begin{equation}
...
Funktion bestimmt.
Bei den abgelesenen Werten wird zwischen den K-alpha-Strahlen, in \autoref{tab:energien_roentgen}, und K-beta-Strahlen,
in \autoref{tab:energien_roentgen2}, unterschieden.
...
\begin{table}[H]
	...
	$E_{Char}$ \dots abgelesener Energiewert in \SI{}{\kilo eV}\\
...
\begin{table}
	\caption[Verzeichnete Energie bei entsprechenden Abstrahlwinkel]
	{Verzeichnete Energie bei entsprechenden Abstrahlwinkel\\
	$\phi$ \dots entsprechender Abstrahlwinkel in \SI{}{\degree}\\
	$E_{Char}$ \dots verzeichneter Energiewert in \SI{}{\kilo eV}}
